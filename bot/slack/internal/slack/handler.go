...
import (
	...
	linearclient "github.com/jae-labs/conCIerge/internal/linear"
...
// replyPR creates a Linear issue, posts the request summary to #concierge-requests, then closes the chat.
func (h *Handler) replyPR(state *conversation.State, prTitle, prURL string) {
	...
	// build summary for #concierge-requests
	...
	// build permalink to the #concierge-requests message
	var link string
	if msgTS != "" {
...
func (h *Handler) handlePRApproval(userID, channelID, messageTS string) {
	...
	if len(matches) < 2 {
		// not a concierge request message
		return
	}
	prNumber, _ := strconv.Atoi(matches[1])
	...
	// thread reply on the #concierge-requests message
	h.api.PostMessage(channelID,
...
// buildRequestSummary creates the blocks posted to #concierge-requests.

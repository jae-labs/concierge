...
| Package | Role |
|---|---|
| `internal/config` | Env var loading and validation; RBAC role sets (user/manager/admin) |
...
| `internal/linear` | Linear GraphQL client for creating tracking issues with priority mapping (high=2, medium=3, low=4) |
| `internal/slack` | Event handler (Socket Mode for dev, HTTP for prod), interaction routing, Block Kit modal definitions, input validation (repo/dns/org), PR approval via reactions |
...
Three role tiers parsed from comma-separated env vars (`SLACK_USER_IDS`, `SLACK_MANAGER_IDS`, `SLACK_ADMIN_IDS`):
...
1. Creates a Linear issue with priority mapping
2. Posts a summary message to `#concierge-requests` (channel ID from `SLACK_REQUESTS_CHANNEL_ID`)
3. Monitors `reaction_added` events for `+1`/`thumbsup` on that message
4. `handlePRApproval()` validates the reactor is an approver, extracts PR URL from message, and processes the approval
...
| [Adding a Resource Type](docs/adding-a-resource-type.md) | 12-step guide for adding new terraform resource support |
...
| New bot-terraform coupling | Path constants section (this file), `iac/terraform/AGENTS.md`, root `AGENTS.md` |

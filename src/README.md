...
Terraform files live in `terraform/` within this monorepo. The bot targets the `conCIerge` repo by default.
...
| `release.yml` | Pushes to `main` that touch `src/**` or the bot workflow files | Builds Linux and macOS release artifacts, creates the next patch release, and refreshes the `latest` release |
